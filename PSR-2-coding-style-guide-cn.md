...
- 类的属性和方法**必须**声明其作用域（`private`、`protected` 以及 `public`）， `abstract` 以及 `final` **必须**声明在作用域之前，而 `static` **必须**声明在作用域之后。
...
需要时，关键词 `abstract` 和 `final`， 必须 在作用域前声明，而 `static` 则 必须 声明在作用域后。
